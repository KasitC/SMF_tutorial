...
Avg_plot <- PlotAvgSMF(MethGR = Methylation[[1]],
                       RegionOfInterest = gr,
                       TFBSs = TFBS)
```
...
```{r}
SM_plot <- PlotSM(MethSM = Methylation[[2]], RegionOfInterest = gr, sorting.strategy = "classical", SortedReads = SortedReads)

StateQuan_plot <- StateQuantificationPlot(SortedReads = SortedReads, states = SingleTFStates())
...
layout <- "
  ...
  CB
"
Avg_plot + SM_plot + StateQuan_plot +
...
```{r}
MethSM_KO <- list(REST_KO = Methylation[[2]][[1]]$DGCHN)
SortedReads_KO <- SortReadsBySingleTF(MethSM = MethSM_KO, TFBS = TFBS, coverage = 20)

SM_plot <- PlotSM(MethSM = MethSM_KO,
                  RegionOfInterest = gr,
                  sorting.strategy = "classical",
                  SortedReads = SortedReads_KO)
...
## Bonus: Integrative analysis with other orthogonal assays
```{r}
source("/g/krebs/barzaghi/Rscripts/CrappyUtils.R")
source('/g/krebs/barzaghi/Rscripts/useful_functionsV1.r')
TFBS <- Load.TFBSs()
unique(TFBS$TF)
TFBS_ChipOK <- TFBS %>% filter(isBound == "TRUE")
REST <- TFBS_ChipOK %>% filter(TF == "REST")
```


```{r}
REST_extend <- resize(REST, 200, fix = "center")
